...
//	TestCheckpointWithoutCRIU          501 when the binary is absent, for checkpoint and restore alike.
//	TestCheckpointDumpsTheRunning      The running session's pid reaches criu dump under the images root; an unknown
...
func TestCheckpointWithoutCRIU(t *testing.T) {
	params := map[string]any{"criu": filepath.Join(t.TempDir(), "criu"), "images_dir": t.TempDir()}
	if status, _, msg := checkpoint(context.Background(), params); status != http.StatusNotImplemented {
		t.Fatalf("checkpoint answered %d %q, want 501", status, msg)
	}
...
func TestCheckpointDumpsTheRunning(t *testing.T) {
	...
	if status, body, msg := checkpoint(context.Background(), params); status != http.StatusOK || len(body.([]map[string]string)) != 0 {
	...
	status, body, msg := checkpoint(context.Background(), params)
	if status != http.StatusOK {
		t.Fatalf("checkpoint answered %d %q", status, msg)
	}
	...
	if got := body.([]map[string]string); !slices.EqualFunc(got, want, func(a, b map[string]string) bool { return a["id"] == b["id"] && a["images_dir"] == b["images_dir"] }) {
		t.Fatalf("checkpoint answered %v, want %v", got, want)
	}
	...
	params["id"] = "p-0"
	if status, _, msg := checkpoint(context.Background(), params); status != http.StatusNotFound {
