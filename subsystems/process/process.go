// Package process runs a command as a session and checkpoints it with CRIU. A command posted to
// /api/v1/process/sessions runs under sh with Linkspan's stdio and is listed until it ends; with stop_on_exit its
...
//	checkpoint    Dumps params.id, or every running session without one, to <params.images_dir>/<id>, the root
...
//	Router        /process/sessions, and checkpoint and restore beside it.
package process
	...
	created, err := (&tasks.Task{ID: id, Kind: kind, Attrs: func(tasks.Task) map[string]string {
		...
		context.AfterFunc(ctx, func() {
			if stopOnExit && slices.ContainsFunc(tasks.Select(kind), func(t tasks.Task) bool { return t.ID == id }) {
				log.Printf("process: %s ended, stopping", id)
				exit()
			}
		})
...
func checkpoint(ctx context.Context, params map[string]any) (int, any, string) {
	bin, err := criu(params)
	if err != nil {
	...
	for _, t := range tasks.Select(kind) {
		...
		cmd.Stdout, cmd.Stderr = os.Stdout, os.Stderr
		log.Printf("process: checkpointing %s to %s", t.ID, images)
