...
// workflow step, starts a stock server on loopback in one shared Python environment under ~/.cybershuttle, built on
// first use, and answers with its address and the token a browser opens it with. A client that committed to a port
// and token ahead may name them; otherwise Linkspan chooses both.
//
//	kind
//	newToken
