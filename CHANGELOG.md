...
- `/api/v1/process/sessions`: a command run under `sh` as a session, listed with its `pid`, which with
  `stop_on_exit` stops Linkspan when it ends, so a batch job ends with its payload.
- `/api/v1/process/checkpoint` and `/api/v1/process/restore`: a running session's process tree dumped
