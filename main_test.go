...
//	TestExampleWorkflowLoads     Every example must name only commands the subsystems export.
...
func TestRoutesFollowConfig(t *testing.T) {
	...
	all := config{"workflow": true, "vscode": true, "jupyter": true, "terminal": true, "filesystem": true, "process": true}
	for _, path := range []string{"/api/v1/vscode/sessions", "/api/v1/jupyter/sessions", "/api/v1/terminal/sessions", "/api/v1/process/sessions"} {
...
func TestExampleWorkflowLoads(t *testing.T) {
	all := config{"vscode": true, "jupyter": true, "terminal": true, "filesystem": true, "process": true}
