...
// Its work is one of four things: a Run, Linkspan's own code, whose error is fatal; a Server, served on the bound
// listener; a Spawn, a child process on the bound port; or a Child, a child process with no port. A child's life
// Linkspan only observes, so its end is recorded and never fatal. Start is the one entry: it binds the address, if any, registers the task and runs it
...
//	             StopAll hangs. Server and Spawn default Addr to loopback on any port. Pid is a Child's, once it
//	             has started. Attrs are the caller's wire fields, computed from the bound task.
...
//	MarshalJSON  The wire object: id, addr, state and error, pid for a started Child, then the attrs.
...
//	             or a Child set it to starting; a repeated id cancels and replaces the earlier task. Run goes on its own
//	             goroutine; the listener closes on cancellation, and a Server after it. A returned error sets
//	             failed and is fatal unless the task is a Spawn or a Child; a panic is an error; a cancelled Run's error is dropped; a nil return is exited. The
