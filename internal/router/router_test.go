...
func ok(context.Context, map[string]any) (int, any, string) { return http.StatusOK, nil, "" }
...
func TestParams(t *testing.T) {
	...
	if call(http.MethodGet, "/x/s", "") != 200 || call(http.MethodPost, "/x/s", `{"a": 1}`) != 200 || call(http.MethodDelete, "/x/s/j-9", "") != 200 {
	...
	if code := call(http.MethodPost, "/x/s", `{"a": "`+strings.Repeat("x", 65<<10)+`"}`); code != http.StatusRequestEntityTooLarge {
...
func TestNestedPrefixes(t *testing.T) {
	...
	for level := 1; level <= depth; level++ {
		...
		for _, route := range []string{"POST " + path + "/items", "DELETE " + path + "/items/{id}", "POST " + path + "/sib/items/signal", "GET " + path + "/shared/items", "GET " + path + "/shared/extra"} {
			want[route] = true
		}
	}
