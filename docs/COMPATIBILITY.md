...
cs-plane launches Linkspan with `--port --workflow --tunnel-enable --tunnel-mode=websocket
--tunnel-websocket-args="--url $CS_LINK_URL"`, adding `devtunnel` to `--tunnel-mode` and
`--tunnel-devtunnel-args="--id … --cluster …"` when the owner delegated a Dev Tunnels account, and exports
...
| Surface | Contract |
|---|---|
| `--version` | A bare `X.Y.Z[.commit]`, the only line on stdout. cs-plane refuses a Linkspan below `0.21.0` by `sort -V`. |
...
| Delegated tunnel | Linkspan only hosts the tunnel and publishes no port, so `LINKSPAN_TUNNEL_HOST_TOKEN` needs only the `host` scope. cs-plane declares only the control port and reaches every other port through `/forward`. |
