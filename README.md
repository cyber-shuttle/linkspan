...
| `process.sessions.select` | Lists the process sessions | |
| `process.sessions.start` | Runs a command under `sh` as a session | `command`, `stop_on_exit` |
| `process.sessions.stop` | Stops one | `id` |
| `process.checkpoint` | Dumps one running session, or all, with CRIU | `id`, `images_dir`, `criu` |
| `process.restore` | Runs a dumped tree as a new session | `images_dir`, `stop_on_exit`, `criu` |
...
| `process` | on | Commands run as sessions, checkpointed and restored with CRIU |
...
| GET | `/api/v1/process/sessions` | `[{"id":"p-<n>","addr":"","state":"<state>","error":"","pid":<pid>,"command":"<command>","stop_on_exit":"<bool>"}]`, ordered by id; `pid` once the command has started |
| POST | `/api/v1/process/sessions` | `201` with one session object; takes `{"command": "<shell command>", "stop_on_exit": <bool>}` |
| DELETE | `/api/v1/process/sessions/{id}` | `{"id":"p-<n>","state":"stopped"}`, `404` for an unknown id |
| POST | `/api/v1/process/checkpoint` | `200` with `[{"id":"p-<n>","images_dir":"<dir>"}]` once CRIU has dumped each; takes `{"id": "<one session>", "images_dir": "<root>", "criu": "<binary>"}`, all optional; `404` for an id that is not running, `501` without CRIU |
| POST | `/api/v1/process/restore` | `201` with one session object running `criu restore`; takes `{"images_dir": "<dir>", "stop_on_exit": <bool>, "criu": "<binary>"}`; `501` without CRIU |
